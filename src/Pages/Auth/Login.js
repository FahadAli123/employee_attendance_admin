    ...
    <div class="w-full h-full flex flex-row bg-white">
      ...
      <div class="min-h-screen md:w-[45%] lg:w-[30%] m-auto flex justify-center items-center drop-shadow-lg ">
        <div class="p-10 w-[100%] border-[1px] -mt-10  rounded-2xl flex flex-col items-center space-y-3 bg-white">
          ...
          <input
            class="p-3 w-full border-[1px] border-slate-500 rounded-lg autofill:bg-white outline-primary"
            placeholder="Email"
            onChange={(event) => setUserName(event.target.value)}
          />
          <div class="w-full">Password</div>
          <input
            class="p-3 w-full border-[1px] border-slate-500 rounded-lg outline-primary "
            placeholder="Password"
            type="password"
            onChange={(event) => setPassword(event.target.value)}
          />

          <div class="w-full flex flex-row justify-end cursor-pointer text-primary hover:text-primaryDark">
            Forgot Password?
          </div>

          <div
            class="w-full cursor-pointer flex items-center justify-center bg-primary rounded-lg p-3 text-white font-bold transition duration-200 hover:bg-primaryDark "
            onClick={Login}
          >
            Login
          </div>
          ...
          <p class="text-center text-black text-xs">
            &copy;2023 Schooler. All rights reserved.
          </p>
        </div>
      </div>
    </div>
